...
__kernel void lstm_gemm(__global float *input,
    ...
    int m = get_local_id(0) + get_group_id(0) * get_local_size(0); //get_global_id(0);//threadIdx.x + blockIdx.x * blockDim.x;
    int n = get_local_id(1) + get_group_id(1) * get_local_size(1); // get_global_id(1); //threadIdx.y + blockIdx.y * blockDim.y;
    int c_wr_idx = idx_2d(m,n,N);
    out_gates[c_wr_idx]  = 0.f;
    for (int k = 0; k < K; k++) {
...
__kernel void lstm_eltwise(__global float* inout_cell,
    ...
    int m = get_global_id(0); // threadIdx.x + blockIdx.x * blockDim.x;
    int n = get_global_id(1); // threadIdx.y + blockIdx.y * blockDim.y;
//    printf("hello from eltwise");
